...
=========
    ...
    This file contains scopes which extend the functionality of the MultiPatrol library. To disable
a certains scope, simply wrap it inside '//! novjass' tags.
...
=========
    ...
    There is no API available, since these are structs and not libraries.
    
    MultiPatrolCommands -> Defines player commands for working with patrol points.
    MultiPatrolOrder    -> Scripts new behaviour for the 'patrol' order. Now each new order will add a patrol point, instead of replacing the unit's patrol path.
    MultiPatrolDisplayOnSelect -> When a unit is selected, it's patrol points are displayed to the player.
...
/* Disabled, as LoP implements it's own command system. */
//! novjass
...
/* Disabled*/
// Does not actually work since this functionality isn't correctly implemented in MultiPatrol.
//! novjass
