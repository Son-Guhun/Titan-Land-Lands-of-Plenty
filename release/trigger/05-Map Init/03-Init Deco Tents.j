...
function InitReforgedTents takes nothing returns nothing
    local rect decoRect = gg_rct_Deco_Tents_Reforged_Rect
    ...
    //! textmacro DecoBuilders_MakeReforgedTents takes name
    set decoTentCount = 1
    loop
    exitwhen decoNumber > LoP_DecoBuilders.$name$DecoLastIndex
        set decoTent = CreateUnit(LoP.NEUTRAL_PASSIVE, 'n03H', xCur, yCur, bj_UNIT_FACING)
        ...
        loop
        exitwhen i >= bj_MAX_STOCK_UNIT_SLOTS or decoNumber > LoP_DecoBuilders.$name$DecoLastIndex
            call AddUnitToStock(decoTent, LoP_DecoBuilders.reforgedRawcodes[decoNumber], 1, 3)
            set i = i + 1
            set decoNumber = decoNumber + 1
        endloop
    ...
    //! runtextmacro DecoBuilders_MakeTents("Special")
    //! runtextmacro DecoBuilders_MakeReforgedTents("Reforged")
